# Workspace Instructions — Gasrulle Theme Pack

> **Dual-file sync rule**: This file (`CLAUDE.md`) and `.github/copilot-instructions.md` contain identical project instructions for Claude Code and GitHub Copilot respectively. Any edit to one file **must** be mirrored to the other so they stay in sync at all times. When updating instructions, palette tables, theme listings, naming conventions, or any other content in either file, always apply the same change to both files in the same edit session.
...
6. **Validate**: Validate the JSON file for syntax errors and check the VS Code Problems panel for invalid/deprecated theme properties and transparency issues.
7. **Document**: Update `README.md`, `CHANGELOG.md`, this file (`CLAUDE.md`), and `.github/copilot-instructions.md` with the new theme's palette table and file listing.
...
### Quality Checks
- Validate all modified theme JSON files for syntax errors and check the VS Code Problems panel for invalid/deprecated properties and transparency requirements
...
2. **Error check**: Validate all modified theme JSON files for syntax errors and check the VS Code Problems panel for invalid/deprecated property warnings.
